...
function getSelectedSupplierDetails() {
	...
	Promise.all([wardrobesDataPromise, cartPromise]).then(function ([wardrobesData, cartData]) {
...
function constructInitialDOM() {
	...
	supplierWardrobesData.forEach((wardrobe, i) => {
		...
		tab.addEventListener("click", () => {
			...
			currentWardrobe = wardrobe.wardrobe_id;
			const selected = supplierWardrobesData.find((w) => w.wardrobe_id === wardrobe.wardrobe_id);
...
function fetchProductVariants(styleMasterCode, gender) {
	...
	Promise.all([variantsPromise, regionalCodePromise]).then(function ([variantsData, regionalData]) {
...
function renderProducts(products) {
	...
	Object.values(grouped).forEach((group) => {
		...
		card.addEventListener("click", function () {
			showLoader();
			fetchProductVariants(item["Style_ID.Style_Master_Code"], item["Style_ID.Gender"]);
		});
...
`;
	...
	if (firstItem["Style_ID.Primary_Image"]) {
		...
		if (imageCache.has(cacheKey)) {
			...
			ZOHO.CREATOR.UTIL.setImageData(mainImg, firstItem["Style_ID.Primary_Image"], function () {
...
<path d="M7.5 15L12.5 10L7.5 5" stroke="#29379E" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
	...
	function showSizeTooltip(size, anchorBtn) {
		...
		const regional_value = match.Size_Chart_Details?.find((r) => r.zc_display_value.includes(buyerRegion));
		...
		const arrowLeft = btnRect.left - tooltipParentRect.left + anchorBtn.offsetWidth / 2;
	...
	function updateSelectedItem() {
		const match = group.find((item) => item.Size?.Size === selectedSize && item["Colors.Hex_Color_Code"] === selectedColor);
		if (!match) return;

		selectedItem = match;

		document.getElementById("pd-product-price").textContent = `${buyerCurrency} ${match.Sell_Price}`;
		document.getElementById("pd-description").textContent = match["Style_ID.Product_Description"] || "";
		...
		if (match["Style_ID.Primary_Image"]) {
			...
			if (imageCache.has(cacheKey)) {
				...
				ZOHO.CREATOR.UTIL.setImageData(mainImg, match["Style_ID.Primary_Image"], function () {
					imageCache.set(cacheKey, mainImg.src);
					mainImgWrapper.classList.remove("loading");
					mainImg.classList.remove("hidden");
					mainPlaceholder.classList.add("hidden");
	...
	function renderColors(forSize) {
		...
		const availableColors = [
			...new Map(
				group.filter((item) => item.Size?.Size === forSize).map((item) => [item["Colors.Hex_Color_Code"], item]),
			).values(),
		];
		...
		availableColors.forEach((item, i) => {
			...
			colorBtn.addEventListener("click", () => {
				Array.from(colorsRow.querySelectorAll(".pd-color-btn")).forEach((b) => b.classList.remove("active"));
	...
	const sizes = [...new Map(group.map((item) => [item.Size?.Size, item])).values()];
	sizes.forEach((item, i) => {
		...
		sizeBtn.addEventListener("click", () => {
			Array.from(sizesRow.children).forEach((b) => b.classList.remove("active"));
	...
	function setSelectorsDisabled(disabled) {
		Array.from(sizesRow.querySelectorAll(".pd-size-btn")).forEach((b) => (b.disabled = disabled));
		Array.from(colorsRow.querySelectorAll(".pd-color-btn")).forEach((b) => (b.disabled = disabled));
	}
	...
	ZOHO.CREATOR.DATA.addRecords(config)
		.then(function (response) {
			if (response.code == 3000) {
				...
				setTimeout(() => {
					const newRow = cartPanelDiv.querySelector(`.cart-item[data-id="${newRecord.ID}"]`);
					if (newRow) {
...
function renderCartDetails(data) {
	...
	filterBar.querySelectorAll(".cart-filter-btn").forEach((btn) => {
		btn.addEventListener("click", () => {
			filterBar.querySelectorAll(".cart-filter-btn").forEach((b) => b.classList.remove("active"));
...
function renderCartItems(data, container, filter) {
	container.innerHTML = "";
	const filtered = filter === "current" ? data.filter((item) => item["Product.Style_Category"]?.ID === currentCategory) : data;
...
function submitCart() {
	...
	ZOHO.CREATOR.DATA.updateRecords(config).then(function (response) {
		if (response.code == 3000) {
			...
			ZOHO.CREATOR.UTIL.navigateParentURL({ action: "open", url: "#Page:My_Basket_Page", window: "same" });
		}
	});
}
	...
	items.forEach((item, idx) => {
		...
		if (!q) {
			...
			while ((p = lo.indexOf(q, s)) !== -1) {
				r += t.slice(s, p) + '<span class="match">' + t.slice(p, p + q.length) + "</span>";
				s = p + q.length;
			}
			r += t.slice(s);
			item.innerHTML = r;
